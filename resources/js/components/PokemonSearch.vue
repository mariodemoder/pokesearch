<template>
    <section class="mx-auto w-full max-w-3xl rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-xl shadow-orange-100 backdrop-blur sm:p-8">
        <header class="mb-6">
            ...
            <p class="mt-2 text-sm text-slate-600">Consumo directo desde Vue con estados de carga, error y exito.</p>
        </header>
...
export default {
    ...
    methods: {
        async buscarPokemon() {
            ...
            try {
                const res = await axios.get(`https://pokeapi.co/api/v2/pokemon/${this.nombre.toLowerCase()}`);
